        ...
        return $table
            ...
            ->actions([
                Action::make('download')
                    ...
                    ->url(fn ($record) => route('export.download', ['exportId' => $record->id]))
                    ->openUrlInNewTab()
                ...
                Action::make('delete')
                    ...
                    ->modalDescription('Вы уверены, что хотите удалить этот экспорт? Файл также будет удален с сервера.')
                    ->modalSubmitActionLabel('Да, удалить')
                    ->action(function ($record) {
                        // Удаляем файлы с диска
                        $filePath = "filament_exports/{$record->id}/{$record->file_name}.xlsx";
                        if (\Illuminate\Support\Facades\Storage::disk($record->file_disk)->exists($filePath)) {
                            \Illuminate\Support\Facades\Storage::disk($record->file_disk)->deleteDirectory("filament_exports/{$record->id}");
                        }
            ...
            ->bulkActions([
                BulkAction::make('delete_selected')
                    ...
                    ->modalDescription('Вы уверены, что хотите удалить выбранные экспорты? Файлы также будут удалены с сервера.')
                    ->modalSubmitActionLabel('Да, удалить')
