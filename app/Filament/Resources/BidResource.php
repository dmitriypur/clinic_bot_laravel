...
class BidResource extends Resource
{
    ...
    {
        return $form->schema([
            Hidden::make("onec_slot_id")->dehydrated(true),
            TextInput::make("full_name_parent")->label("ФИО родителя"),
                ...
                ->rules([
                    fn(): \Closure => function (
                        ...
                        if (request()->isMethod("POST") && !$value) {
                            $fail(
                                "Поле ФИО ребенка обязательно для заполнения.",
                            );
                        }
                    },
                ])
            ...
            Select::make("clinic_id")
                ...
                ->options(function (callable $get) {
                    ...
                    if ($user->hasRole("partner")) {
                        ...
                        if (
                            $clinic &&
                            $clinic->cities->contains("id", $cityId)
                        ) {
                    ...
                    return \App\Models\Clinic::whereHas("cities", function (
                        $query,
                    ) use ($cityId) {
                ...
                ->options(function (callable $get) {
                    ...
                    if ($user->hasRole("partner")) {
                        ...
                        if ($clinic) {
                            ...
                            return $query
                                ->get()
                                ->mapWithKeys(function ($branch) use ($clinic) {
                                    return [
                                        $branch->id =>
                                            $clinic->name .
                                            " - " .
                    ...
                    $query = \App\Models\Branch::with("clinic")->where(
                        "city_id",
                        $cityId,
                    );
            ...
            Select::make("doctor_id")
                ...
                ->options(function (callable $get) {
                    ...
                    if ($branchId) {
                        ...
                        $query->whereHas("branches", function ($q) use (
                            $branchId,
                        ) {
                        ...
                        $query->whereHas("clinics", function ($q) use (
                            $clinicId,
                        ) {
                        ...
                        $query->whereHas("clinics", function ($q) use (
                            ...
                            $q->whereHas("cities", function ($cityQuery) use (
                                $cityId,
                            ) {
            ...
            Select::make("cabinet_id")
                ...
                ->options(function (callable $get) {
                    ...
                    return \App\Models\Cabinet::where(
                        "branch_id",
                        $branchId,
                    )->pluck("name", "id");
                }),

            Toggle::make("send_to_1c")->label("Отправить в 1С")->hidden(),
            ...
            Select::make("status_id")
                ...
                ->relationship(
                    "status",
                    "name",
                    fn($query) => $query->where("type", "bid"),
                )
                ->searchable()
                ->preload()
                ->live()
                ->default(function () {
                    $newStatus = \App\Models\ApplicationStatus::where(
                        "slug",
                        "new",
                    )->first();
            ...
            DateTimePicker::make("appointment_datetime")
                ...
                ->afterStateUpdated(function (
                    ...
                    if ($appointmentDatetime) {
                        ...
                        $carbon =
                            $appointmentDatetime instanceof Carbon
                                ? $appointmentDatetime
                                    ->copy()
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make("status.name")
                    ...
                    ->badge()
                    ->color(
                        fn($record) => $record->status?->getBadgeColor() ??
                            Color::Gray,
                    )
                    ->searchable()
                    ->toggleable()
